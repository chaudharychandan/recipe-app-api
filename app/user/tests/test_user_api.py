...
class PublicUserApiTests(TestCase):
    ...
    def test_create_token_invalid_credential(self):
        """Test if token is not created when invalid credentials are given"""
        payload1 = {
            'email': 'test1@gmail.com',
            'password': 'test1',
        }
    ...
    def test_create_token_no_user(self):
        """Test if token is not created when user doesn't exist"""
        payload = {
            'email': 'test1@gmail.com',
            'password': 'test1',
        }
...
class PrivateUserAPITests(TestCase):
    ...
    def test_post_me_not_allowed(self):
        """Test if post if not allowed on the me url"""
        res = self.client.post(ME_URL, {})

        self.assertEqual(res.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)

    def test_update_user_profile(self):
        """Test updating the user profile for an authenicated user"""
        payload = {
